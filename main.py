...
def start(message):
    mess = f'Привет, {message.from_user.first_name} {message.from_user.last_name}! Что бы получить ссылку для отправки сообщения в WhatsApp без добавления в список контактов, просто пришлите номер в международном формате (например +7(ХХХ)ХХХ-ХХ-ХХ, +7ХХХХХХХХХХ):'
